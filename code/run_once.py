...
if args.geometry in ['left-volume', 'cx-volume', 'cic-volume', 'volume']: 
    ...
    Yk0, yk1 = run_wrappers.generate_covariates(curr_map=args.maptag, src_polydata=src_polydata, trg_polydata=trg_polydata, nspins=3000, datasource=args.datasource)
...
for iter_id in range(niterations):
    run_wrappers.run_one_iteration(
            **set_up_analysis,
            curr_iter=iter_id)
