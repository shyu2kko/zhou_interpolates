...
def get_training_set(mesh, sphere, pct_split, n_iters, parcellation_name=None, method='random', sphere_foci=None, seed=321):
    ...
    if method == 'fibonacci':
        ...
        unknown_coords[:,:,curr_iter] = consider_points
    ...
    if method == 'focaldiffuse':
        ...
        sample_counts = np.array(rel_size * n_train) # rounds to nearest integer
...
def _get_distance_between(X0, x1, method = 'euclidean'):
    '''
...
def get_empirical_variogram_fast(x,X,D,nh,h_bounds=0):
    '''
    calculate the empirical variogram from
    ...
    h = np.linspace(0.000001,max_lag,nh)
...
def fit_empirical_variogram(x,D,X,smoothing=False, method = 'best', fast=True, nh=25,h_bounds=0, summary=True, plotit=False, filename=None):
    ...
    if fast: u, ev, counts = _get_empirical_variogram_fast(x=x,X=X,D=D,nh=nh,h_bounds=h_bounds)
